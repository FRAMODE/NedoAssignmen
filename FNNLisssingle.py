...
datatrain = Datadata()
train_dataloader = DataLoader(datatrain,batch_size=50,shuffle=False) 
...
optimizer = optim.Adam(net.parameters(), lr=conflr)
...
plt.ylim(-1,1)
plt.yscale('linear')
plt.savefig(f"FNNfig/FNN_result_{date[0]}_{date[1]}.png")
plt.show()
